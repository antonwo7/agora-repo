...
class GroupsAdminModel {
		...
		$fields = [
			...
			'general' => [
				...
				'active'        => [
					...
					'value'       => '',
					'description' => __( 'Choose if the category is active and available on the site', 'lisfinity-core' ),
					'type'        => 'select',
					'choices'     => [
				...
				'minimum_price' => [
					'key'         => $count ++,
					'label'       => __( 'Minimum Listing Price', 'lisfinity-core' ),
					'value'       => '',
					'description' => __( 'Set the minimum price for the listings submitted in the category or leave empty to disable', 'lisfinity-core' ),
					'type'        => 'text',
					'placeholder' => '0',
					'additional'  => [
						'class' => 'w-48%',
				...
				'disable_price' => [
					'key'         => $count ++,
					'label'       => __( 'Disable Price Step', 'lisfinity-core' ),
					'value'       => '',
					'description' => __( 'Disable price submission step for this category in the listing submission form', 'lisfinity-core' ),
					'type'        => 'select',
					'choices'     => [
						'no'  => __( 'No', 'lisfinity-core' ),
						'yes' => __( 'Yes', 'lisfinity-core' ),
				...
				'premium'       => [
					...
					'value'       => '',
					'description' => __( 'Listings from this category will be available for premium/member users only.', 'lisfinity-core' ),
					'type'        => 'select',
					'choices'     => [
						'no'  => __( 'No', 'lisfinity-core' ),
						'yes' => __( 'Yes', 'lisfinity-core' ),
	 ...
	 * @return array
	 */
	public function get_options( $include_common = false, $hide_inactive = true ) {
		if ( $include_common ) {
			$options[] = [
				"single_name" => "Common",
				"plural_name" => "Commons",
		...
		if ( $hide_inactive ) {
			foreach ( $options as $key => $value ) {
				if ( isset( $value['active'] ) && 'no' === $value['active'] ) {
					unset( $options[ $key ] );
				}
			}
		}

...
class RegisterModel {
	...
	public function process_registration( $data ) {
		global $wpdb;
		$unlimited_packages_enabled = false;
		$result                     = [];
		...
		if ( ! empty( $data['code'] ) ) {
			...
			if ( empty( $codes ) ) {
				$result['error']                 = true;
				$result['error_message']['code'] = __( 'The code is not valid', 'lisfinity-core' );
			}

			if ( ! empty( $codes ) ) {
				$code = $codes[0];

				carbon_set_post_meta( $code, 'status', 'used' );
				$unlimited_packages_enabled = true;
			}
		}
		...
		if ( $unlimited_packages_enabled ) {
			carbon_set_user_meta( $user_id, 'tierschutz', true );
		}
		...
		// unlimited package.
		$unlimited_package = lisfinity_get_option( 'auth-unlimited-package' );

		if ( $unlimited_packages_enabled && ! empty( $unlimited_package ) ) {
			$package_controller = new PackageController();

			$package_obj = wc_get_product( $unlimited_package );
			...
			if ( ! empty( $package_obj ) ) {
				$values = [
					...
					carbon_get_post_meta( $unlimited_package, 'package-products-duration' ),
					// type of the package.
					'payment_package',
				...
				$prefix     = '';
				$promotions = carbon_get_post_meta( $package_id, "{$prefix}package-promotions" );

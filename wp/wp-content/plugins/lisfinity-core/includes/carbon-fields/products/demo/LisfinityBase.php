<?php
if ( ! class_exists( "LisfinityBase" ) ) {
	class LisfinityBase {
		public $key = "B9C34B35013EBF1F";
		private $product_id = "1";
		private $product_base = "lisfinity-core";
		private $server_host = "https://wordpress-411969-1704007.cloudwaysapps.com/wp-json/licensor/";
		...
		function __construct( $plugin_base_file = '' ) {
			...
			if ( $this->hasCheckUpdate ) {
				if ( function_exists( "add_action" ) ) {
					add_action( 'admin_post_lisfinity-core_fupc', function () {
					...
					if ( $this->isTheme ) {
						...
						add_filter( 'plugin_row_meta', function ( $links, $plugin_file ) {
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=lisfinity-core_fupc' ) . "'>Update Check</a>";
							}
		...
		final function _CheckWPPlugin( $purchase_key, &$error = "", &$responseObj = null ) {
			$responseObj = (object) [
			'license_title' => 'Fulltime license',
			'license_key' => get_option( 'Lisfinity_lic_Key' ),
					...
					if ( ! empty( $response->status ) ) {
						if ( ! empty( $response->data ) ) {
							...
							if ( $licenseObj->is_valid ) {
								...
								$responseObj->domain             = $licenseObj->domain;
								$this->SaveWPResponse( $responseObj );
								unset( $responseObj->next_request );
